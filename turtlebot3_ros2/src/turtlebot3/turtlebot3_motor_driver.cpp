...
const uint8_t DXL_MOTOR_ID_FRONTRIGHT = 3;
const uint8_t DXL_MOTOR_ID_FRONTLEFT = 4;   // ID of front motor
...
: left_wheel_id_(DXL_MOTOR_ID_LEFT),
  ...
  frontleft_wheel_id_(DXL_MOTOR_ID_FRONTLEFT),
  frontright_wheel_id_(DXL_MOTOR_ID_FRONTRIGHT),
  torque_(false)
{
}
...
bool Turtlebot3MotorDriver::init(void)
{
  ...
  sync_write_param.xel[FRONTRIGHT].id = frontright_wheel_id_;
  sync_write_param.xel[FRONTLEFT].id = frontleft_wheel_id_;
  ...
  sync_read_param.xel[FRONTRIGHT].id = frontright_wheel_id_;
  sync_read_param.xel[FRONTLEFT].id = frontleft_wheel_id_;
...
{
  return (dxl.ping(DXL_MOTOR_ID_LEFT) == true && dxl.ping(DXL_MOTOR_ID_RIGHT) == true && dxl.ping(DXL_MOTOR_ID_FRONTRIGHT) == true && dxl.ping(DXL_MOTOR_ID_FRONTLEFT) == true);
}
...
bool Turtlebot3MotorDriver::set_torque(bool onoff)
{
  ...
  sync_write_param.xel[FRONTRIGHT].data[0] = onoff;
  sync_write_param.xel[FRONTLEFT].data[0] = onoff;
...
bool Turtlebot3MotorDriver::get_torque()
{
  if(dxl.readControlTableItem(TORQUE_ENABLE, left_wheel_id_) == true
    ...
    && dxl.readControlTableItem(TORQUE_ENABLE, frontright_wheel_id_) == true
    && dxl.readControlTableItem(TORQUE_ENABLE, frontleft_wheel_id_) == true){
    torque_ = true;
  }else{
    torque_ = false;
  }
...
bool Turtlebot3MotorDriver::read_present_position(int32_t &left_value, int32_t &right_value, int32_t &frontright_value, int32_t &frontleft_value)
{
  bool ret = false;
  ...
  if(dxl.syncRead(sync_read_param, read_result)){
    ...
    memcpy(&frontright_value, read_result.xel[FRONTRIGHT].data, read_result.xel[FRONTRIGHT].length);
    memcpy(&frontleft_value, read_result.xel[FRONTLEFT].data, read_result.xel[FRONTLEFT].length);
    ret = true;
  }
...
bool Turtlebot3MotorDriver::write_position(int32_t left_angle, int32_t right_angle)
{
  bool ret = false;
  
  sync_write_param.addr = 116;
  sync_write_param.length = 4;
  memcpy(sync_write_param.xel[FRONTRIGHT].data, &right_angle, sync_write_param.length);
  memcpy(sync_write_param.xel[FRONTLEFT].data, &left_angle, sync_write_param.length);
...
bool Turtlebot3MotorDriver::write_profile_acceleration(uint32_t left_value, uint32_t right_value, uint32_t frontright_value, uint32_t frontleft_value)
{
  bool ret = false;
  ...
  memcpy(sync_write_param.xel[LEFT].data, &left_value, sync_write_param.length);
  memcpy(sync_write_param.xel[RIGHT].data, &right_value, sync_write_param.length);
  memcpy(sync_write_param.xel[FRONTRIGHT].data, &frontright_value, sync_write_param.length);
  memcpy(sync_write_param.xel[FRONTLEFT].data, &frontleft_value, sync_write_param.length);
  if(dxl.syncWrite(sync_write_param)){
    ret = true;
  }
...
bool Turtlebot3MotorDriver::read_profile_acceleration(uint32_t &left_value, uint32_t &right_value, uint32_t &frontright_value, uint32_t &frontleft_value)
{
  bool ret = false;
...
bool Turtlebot3MotorDriver::control_motors(const float wheel_separation, float linear_value, float angular_value)
{
  ...
  // If linear_value is zero, set both wheel velocities to zero and return
  if (linear_value == 0) {
    dxl_comm_result = write_velocity(0, 0);
    return dxl_comm_result;
  }

  // Otherwise, calculate and set wheel velocities based on linear and angular values
  ...
  // Write wheel velocities to motors
