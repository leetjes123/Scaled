...
static int32_t goal_steering_angle[MortorLocation::MOTOR_NUM_MAX] = {0, 0};

static void update_goal_velocity_from_3values(void);
static void steering_angle(void);
...
enum ControlTableItemAddr{
  ...
  //New Addresses for Steering angles
  ADDR_FRONTLEFT_ANGLE          = 350,
  ADDR_FRONTRIGHT_ANGLE         = 354
...
typedef struct ControlItemVariables{
  ...
  int32_t frontright_angle;
  int32_t frontleft_angle;
...
void TurtleBot3Core::begin(const char* model_name)
{
  ...
  // Items to control steering servos
  dxl_slave.addControlItem(ADDR_FRONTRIGHT_ANGLE, control_items.frontright_angle);
  dxl_slave.addControlItem(ADDR_FRONTLEFT_ANGLE, control_items.frontleft_angle);
...
void TurtleBot3Core::run()
{
  ...
  if (millis()-pre_time_to_control_motor >= INTERVAL_MS_TO_CONTROL_MOTOR)
  {
    ...
    if(true){
      ...
      motor_driver.write_position(goal_steering_angle[LEFT], goal_steering_angle[RIGHT]);    
    }
  }  
}
...
{
  goal_steering_angle[LEFT] = control_items.frontleft_angle;
  goal_steering_angle[RIGHT] = control_items.frontright_angle;
}
...
void update_motor_status(uint32_t interval_ms)
{
  ...
  if(millis() - pre_time >= interval_ms){
    ...
    if(get_connection_state_with_motors() == true){
      motor_driver.read_present_position(control_items.present_position[MortorLocation::LEFT], control_items.present_position[MortorLocation::RIGHT], control_items.present_position[MortorLocation::FRONTRIGHT], control_items.present_position[MortorLocation::FRONTLEFT]);
...
static void dxl_slave_write_callback_func(uint16_t item_addr, uint8_t &dxl_err_code, void* arg)
{
  ...
  switch(item_addr)
  {
    ...
    case ADDR_PROFILE_ACC_R:
      if(get_connection_state_with_motors() == true)
        motor_driver.write_profile_acceleration(control_items.profile_acceleration[MortorLocation::LEFT], control_items.profile_acceleration[MortorLocation::RIGHT], control_items.profile_acceleration[MortorLocation::FRONTRIGHT], control_items.profile_acceleration[MortorLocation::FRONTLEFT]);
      break;
...
void test_motors_with_buttons(uint8_t buttons)
{
  ...
  if(get_connection_state_with_motors() == true){
    motor_driver.read_present_position(current_tick[MortorLocation::LEFT], current_tick[MortorLocation::RIGHT], current_tick[MortorLocation::FRONTRIGHT], current_tick[MortorLocation::FRONTLEFT]);
  }
